...
- compare_csvs.py: Python script analyzing semantic similarity for two CSVs
...
## System Requirements
Pytorch is an open source deep learning framework. One of its capabilities is enabling LLMs, such as BERT, to be run on a local computer. Pytorch as well as a multitude of supported libraries can be installed at no cost. 
...
- Computer with Nvidia GPU that supports CUDA (optional if you will not be doing heavy computing)
